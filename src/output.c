...
extern char* pHeaderLine1;
extern char* pHeaderLine2;
extern char* pHeaderLine3;

public char underHeaderLine1[4096];
public char underHeaderAttr1[4096];
public char underHeaderLine2[4096];
public char underHeaderAttr2[4096];
public char underHeaderLine3[4096];
public char underHeaderAttr3[4096];
public char* pUnderHeaderLine1 = underHeaderLine1;
public char* pUnderHeaderAttr1 = underHeaderAttr1;
public char* pUnderHeaderLine2 = underHeaderLine2;
public char* pUnderHeaderAttr2 = underHeaderAttr2;
public char* pUnderHeaderLine3 = underHeaderLine3;
public char* pUnderHeaderAttr3 = underHeaderAttr3;
...
public void unput_header() {
	if (useHeaders) {
		home();
		set_line_buffer(pUnderHeaderLine1);
		set_attr_buffer(pUnderHeaderAttr1);
		put_line();
		set_line_buffer(pUnderHeaderLine2);
		set_attr_buffer(pUnderHeaderAttr2);
		put_line();
		set_line_buffer(pUnderHeaderLine3);
		set_attr_buffer(pUnderHeaderAttr3);
		put_line();
	}
}
...
public void put_header() {
	...
	if (pHeaderLine1 && pHeaderLine2 && pHeaderLine3) {
		if (useHeaders) {
			home();
			for (j = 0; j < 3; ++j) {
				if (j == 0) {
					headerLine = pHeaderLine1;
				} else if (j == 1) {
					headerLine = pHeaderLine2;
				} else {
					headerLine = pHeaderLine3;
				}
				headerLine += hshift;
				for (i = 0;  i < get_screen_width();  i++) {
					if ((c = *headerLine) != '\n') {
						putchr(c);
						headerLine++;
					} else {
